        ...
        <div className="flex-1 p-8 overflow-auto flex flex-col relative animate-fade-in-up">
          <div className="max-w-6xl mx-auto w-full flex flex-col h-full">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
              <div className="flex bg-surface-card p-1 rounded-[12px] border border-hairline">
                <button 
                  onClick={() => setActiveTab('drafts')}
                  className={`px-6 py-2 text-sm font-semibold rounded-[8px] transition-all ${activeTab === 'drafts' ? 'bg-primary text-white' : 'text-charcoal hover:text-ink hover:bg-surface-bone'}`}
                >
                  Drafts
                </button>
                <button 
                  onClick={() => setActiveTab('scheduled')}
                  className={`px-6 py-2 text-sm font-semibold rounded-[8px] transition-all ${activeTab === 'scheduled' ? 'bg-primary text-white' : 'text-charcoal hover:text-ink hover:bg-surface-bone'}`}
                >
                  Scheduled
                </button>
                <button 
                  onClick={() => setActiveTab('history')}
                  className={`px-6 py-2 text-sm font-semibold rounded-[8px] transition-all ${activeTab === 'history' ? 'bg-primary text-white' : 'text-charcoal hover:text-ink hover:bg-surface-bone'}`}
                >
                  History
                </button>
              </div>
            ...
            <div className="bg-surface-card border border-hairline rounded-[16px] overflow-hidden flex flex-col flex-1">
              <div className="overflow-auto flex-1">
                <table className="w-full text-left border-collapse min-w-[800px]">
                  ...
                  <tr>
                    <th className="px-6 py-4 text-xs font-semibold text-charcoal uppercase tracking-wider">Name</th>
                    <th className="px-6 py-4 text-xs font-semibold text-charcoal uppercase tracking-wider">Target</th>
                    <th className="px-6 py-4 text-xs font-semibold text-charcoal uppercase tracking-wider">Sent / Read</th>
                    <th className="px-6 py-4 text-xs font-semibold text-charcoal uppercase tracking-wider">Date</th>
                    <th className="px-6 py-4 text-xs font-semibold text-charcoal uppercase tracking-wider text-right">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-hairline">
                  {filteredBroadcasts.length === 0 ? (
                    <tr>
                      <td colSpan="5" className="px-6 py-12 text-center text-mute">
                        No broadcasts found in this category.
                      </td>
                    </tr>
                  ) : filteredBroadcasts.map(b => (
                    <tr key={b.id} className="hover:bg-surface-bone transition-colors group cursor-pointer">
  ...
  const renderCreationFlow = () => {
    return (
      <div className="flex flex-col h-full bg-transparent">
        {/* Stepper Header */}
        <div className="flex items-center justify-between px-8 py-6 border-b border-hairline shrink-0 relative z-10">
          <div className="flex items-center gap-4">
            <button 
              onClick={() => setView('list')}
              className="p-1.5 hover:bg-surface-bone rounded-full text-charcoal hover:text-ink transition-colors"
            >
              <ArrowLeft size={20} />
            </button>
            ...
            <div className="flex items-center gap-2">
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full ${step === 1 ? 'bg-surface-bone text-primary font-medium' : step > 1 ? 'text-ink cursor-pointer hover:bg-surface-bone' : 'text-mute'}`} onClick={() => step > 1 && setStep(1)}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step === 1 ? 'bg-primary text-white' : step > 1 ? 'bg-ink text-white' : 'bg-surface-bone text-mute'}`}>1</span>
                Channel
              </div>
              <ChevronRight size={16} className="text-mute" />
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full ${step === 2 ? 'bg-surface-bone text-primary font-medium' : step > 2 ? 'text-ink cursor-pointer hover:bg-surface-bone' : 'text-mute'}`} onClick={() => step > 2 && setStep(2)}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step === 2 ? 'bg-primary text-white' : step > 2 ? 'bg-ink text-white' : 'bg-surface-bone text-mute'}`}>2</span>
                Content
              </div>
              <ChevronRight size={16} className="text-mute" />
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full ${step === 3 ? 'bg-surface-bone text-primary font-medium' : 'text-mute'}`}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step === 3 ? 'bg-primary text-white' : 'bg-surface-bone text-mute'}`}>3</span>
                Settings
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button className="button-outline border-transparent hover:border-hairline bg-transparent">Save Draft</button>
            {step < 3 ? (
              <button 
        ...
        <div className="flex-1 overflow-auto bg-transparent">
          ...
          {step === 1 && (
            <div className="max-w-4xl mx-auto py-12 px-8">
              <div className="text-center mb-10">
                <h2 className="text-4xl font-bold font-display text-ink leading-none mb-3">Choose a Channel</h2>
                <p className="text-charcoal text-base">Select the platform where you want to send this broadcast.</p>
              </div>
                ...
                {CHANNELS.map(channel => (
                  ...
                  >
                    <div className="flex flex-col items-center text-center">
                      ...
                      <h3 className="text-lg font-bold font-display text-ink mb-2">{channel.name}</h3>
                      <p className="text-sm text-charcoal leading-relaxed">{channel.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {step === 2 && (
            <div className="flex h-full">
              {/* Left: Editor */}
              <div className="flex-1 border-r border-hairline bg-surface-card p-8 flex flex-col">
                <div className="mb-6">
                  <h2 className="text-2xl font-bold font-display text-ink">Message Content</h2>
                  <p className="text-sm text-charcoal mt-1">Design your broadcast flow. Add text, buttons, or images.</p>
                </div>
                
                <div className="flex-1 bg-canvas rounded-[16px] border border-hairline p-6 relative flex flex-col items-center">
                  {selectedChannel === 'email' ? (
                    <div className="w-[600px] bg-surface-card rounded-[16px] shadow-2xl border border-hairline overflow-hidden mx-auto flex flex-col h-full">
                      <div className="bg-surface-bone border-b border-hairline px-4 py-3 flex items-center gap-2 shrink-0">
                        <Mail size={16} className="text-ink" />
                        <span className="text-sm font-bold text-ink uppercase tracking-wider">Email Composer</span>
                      </div>
                      <div className="p-5 flex flex-col flex-1 gap-4">
                        <div>
                          <label className="block text-xs font-bold text-mute mb-1 uppercase tracking-wide">Subject Line</label>
                          <input 
                            type="text"
                            value={emailSubject}
                        ...
                        <div className="flex-1 flex flex-col">
                          <label className="block text-xs font-bold text-mute mb-1 uppercase tracking-wide">Email Body (HTML/Text)</label>
                          <textarea 
                            className="flex-1 w-full text-sm text-ink border border-hairline rounded-[12px] p-4 resize-none focus:outline-none focus:border-hairline-strong font-mono bg-canvas"
                    ...
                    <div className="w-[450px] bg-surface-card rounded-[16px] shadow-2xl border border-hairline overflow-hidden mx-auto flex flex-col h-full">
                      ...
                      <div className="p-5 flex flex-col flex-1 gap-4 overflow-y-auto">
                        <div>
                          <label className="block text-xs font-bold text-mute mb-1 uppercase tracking-wide">Select Template</label>
                          <select 
                            className="text-input"
                            onChange={(e) => {
                        ...
                        {selectedTemplate && (
                          <div className="flex-1 flex flex-col gap-4">
                            <label className="block text-xs font-bold text-mute uppercase tracking-wide">Template Variables</label>
                            {selectedTemplate.components.map((comp, idx) => {
                                if (comp.type === 'BODY' && comp.example?.body_text?.[0]) {
                                  ...
                                  return comp.example.body_text[0].map((ex, vIdx) => (
                                    <div key={`body-${vIdx}`}>
                                      <label className="block text-xs font-medium text-charcoal mb-1">Variable {'{{' + (vIdx + 1) + '}}'} (e.g., "{ex}")</label>
                                      <input 
                                        type="text"
                            ...
                            {Object.keys(mapping).length === 0 && (
                              <p className="text-sm text-mute italic">This template requires no variables.</p>
                            )}
                          </div>
                        )}
                      </div>
              ...
              {/* Right: Preview */}
              <div className="w-[400px] bg-canvas p-8 flex flex-col items-center border-l border-hairline">
                ...
                {selectedChannel === 'email' ? (
                  <div className="w-full h-[500px] bg-surface-card rounded-[16px] shadow-2xl border border-hairline overflow-hidden flex flex-col mt-4">
                    <div className="bg-surface-bone border-b border-hairline px-4 py-3 flex flex-col gap-1 shrink-0">
                      <div className="text-sm"><span className="font-medium text-charcoal w-16 inline-block">From:</span> <span className="text-ink">hello@broadcastpilot.com</span></div>
                      <div className="text-sm"><span className="font-medium text-charcoal w-16 inline-block">To:</span> <span className="text-ink">subscriber@example.com</span></div>
                      <div className="text-sm"><span className="font-medium text-charcoal w-16 inline-block">Subject:</span> <span className="text-ink font-medium">{emailSubject || '(No Subject)'}</span></div>
                    </div>
          ...
          {step === 3 && (
            <div className="max-w-4xl mx-auto py-10 px-8 flex gap-8">
              ...
              <div className="flex-1 space-y-8">
                ...
                <div className="bg-surface-card border border-hairline rounded-[16px] p-6">
                  <h3 className="text-xl font-bold font-display text-ink mb-4 flex items-center gap-2">
                    <Users size={20} className="text-charcoal" /> Target Audience
                  </h3>
                  
                  {selectedChannel === 'email' ? (
                    <div className="p-4 bg-canvas border border-hairline rounded-[12px]">
                      <label className="block text-sm font-medium text-ink mb-2">Send Email To:</label>
                      <select 
                    ...
                    <div className="p-4 bg-canvas border border-hairline rounded-[12px]">
                      <label className="block text-sm font-medium text-ink mb-2">Send SMS To:</label>
                      <select 
                        value={condition}
                        onChange={(e) => setCondition(e.target.value)}
                        className="text-input"
                      ...
                      <p className="text-xs text-charcoal mt-2">Texts will only be sent to leads that have a valid phone number on file.</p>
                    </div>
                  ) : (
                    <div className="p-4 bg-canvas border border-hairline rounded-[12px]">
                      <label className="block text-sm font-medium text-ink mb-2">Send WhatsApp To:</label>
                      <select 
                        value={condition}
                        onChange={(e) => setCondition(e.target.value)}
                        className="text-input"
                      ...
                      <p className="text-xs text-charcoal mt-2">Messages will only be sent to leads matching this status.</p>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-surface-card border border-hairline rounded-[16px] p-6">
                  <h3 className="text-xl font-bold font-display text-ink mb-4 flex items-center gap-2">
                    <Settings size={20} className="text-charcoal" /> Broadcast Settings
                  </h3>
                  ...
                  <div className="space-y-6">
                    {selectedChannel !== 'email' && (
                      <div>
                        ...
                        <p className="text-xs text-charcoal mt-1">Make sure you comply with the platform's 24-hour messaging rule.</p>
                      </div>
                    )}

                    <div>
                      <label className="block text-sm font-medium text-ink mb-3">Schedule</label>
                      <div className="space-y-3">
                        <label className="flex items-center gap-3 cursor-pointer">
                          <input 
                            ...
                            className="w-4 h-4 text-primary border-hairline focus:ring-primary" 
                          />
                          <span className="text-sm text-ink font-medium">Start sending now</span>
                        </label>
                        <label className="flex items-center gap-3 cursor-pointer">
                          <input 
                            type="radio" 
                            ...
                            className="w-4 h-4 text-primary border-hairline focus:ring-primary" 
                          />
                          <span className="text-sm text-ink font-medium">Schedule for later</span>
                        </label>
                      </div>
                      
                      {schedule === 'later' && (
                        <div className="mt-4 p-4 bg-canvas border border-hairline rounded-[12px] flex items-center gap-4">
                          <div className="flex-1">
              ...
              <div className="w-[280px]">
                <div className="bg-surface-card border border-hairline rounded-[16px] p-5 sticky top-8">
                  <h4 className="font-bold font-display text-ink mb-4 text-lg">Summary</h4>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center pb-3 border-b border-hairline">
                      <span className="text-sm text-charcoal">Channel</span>
                      <span className="text-sm font-medium text-ink capitalize">{selectedChannel}</span>
                    </div>
                    <div className="flex justify-between items-center pb-3 border-b border-hairline">
                      <span className="text-sm text-charcoal">Target Reach</span>
                      <span className="text-sm font-bold text-primary">~1,240 users</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-charcoal">Cost</span>
                      <span className="text-sm font-medium text-ink">Free</span>
                    </div>
                  </div>
                  <button 
                    onClick={handleSend}
                    disabled={isSending}
                    className="w-full mt-6 button-primary justify-center"
                  >
