    ...
    <div className="flex flex-col h-full w-full bg-[#f4f5f7] font-sans">
      ...
      <div className="flex items-center justify-between px-8 py-6 bg-white border-b border-gray-100 shadow-sm z-10">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage your leads and assignments</p>
        </div>
        <div className="flex gap-3">
          <button 
            onClick={() => setShowCreateModal(true)}
            className="px-4 py-2 border border-gray-200 bg-white text-gray-700 font-medium rounded-lg hover:bg-gray-50 hover:border-gray-300 transition-all text-sm shadow-sm flex items-center gap-2"
          >
            <Plus size={16} />
            New Contact
          </button>
          <button 
            ...
            className="px-4 py-2 bg-[#0070d1] text-white font-medium rounded-lg hover:bg-blue-700 hover:shadow-md transition-all text-sm shadow-sm flex items-center gap-2"
          >
            <Upload size={16} />
            Import
          </button>
      ...
      <div className="flex-1 overflow-auto p-8 flex flex-col relative">
        {initialLoad ? (
          <div className="m-auto flex items-center justify-center">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#0070d1]"></div>
          </div>
        ) : leads.length === 0 ? (
          <div className="m-auto flex flex-col items-center justify-center max-w-md text-center py-10 animate-fade-in-up">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-3 tracking-tight">No contacts yet</h2>
            <p className="text-gray-500 text-sm mb-8 leading-relaxed">
              Upload your external contacts to start assigning them to agents and using them in your marketing broadcasts.
            </p>
            <button 
              onClick={() => setShowImportModal(true)}
              className="px-6 py-2.5 bg-[#0070d1] text-white font-medium rounded-lg hover:bg-blue-700 hover:shadow-md transition-all text-sm flex items-center gap-2"
            >
              <Upload size={16} />
              Import Contacts
            </button>
          ...
          <div className="w-full max-w-7xl mx-auto flex flex-col h-full animate-fade-in-up">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                <select
                  ...
                  className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-sm"
                >
            ...
            {/* Table Card */}
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm flex flex-col flex-1">
                ...
                <table className="w-full text-left border-collapse min-w-[800px]">
                  <thead className="bg-gray-50/80 border-b border-gray-200 sticky top-0 z-10 backdrop-blur-sm">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider w-48 text-right">Action</th>
                    </tr>
                  </thead>
                        ...
                        <tr key={lead.id} className="hover:bg-blue-50/30 transition-colors group">
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 text-blue-600 flex items-center justify-center font-bold text-sm shrink-0 border border-blue-200/50">
                          ...
                          <td className="px-6 py-4 text-sm text-gray-600 font-medium tracking-wide">{lead.phone}</td>
                          {isOwner ? (
                            <td className="px-6 py-4">
      ...
      {/* Import Modal */}
      {showImportModal && (
        <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in-up" style={{ animationDuration: '0.2s' }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100 transform scale-100">
            <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <h3 className="text-lg font-bold text-gray-900">Import Contacts</h3>
              <button onClick={resetModal} className="text-gray-400 hover:text-gray-700 transition-colors p-1 rounded-md hover:bg-gray-100">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
              </button>
            </div>
            <div className="p-6">
              <div className="flex bg-gray-100/80 rounded-xl p-1.5 mb-6">
                <button 
                  onClick={() => setUploadMode('file')}
                  className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${uploadMode === 'file' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
                >
                  File Upload
                </button>
                <button 
                  onClick={() => setUploadMode('sheet')}
                  className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${uploadMode === 'sheet' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
                >
                  Google Sheet
                </button>
                <button 
                  onClick={() => setUploadMode('meta')}
                  className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${uploadMode === 'meta' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
                >
                  Meta Ads
                </button>
              </div>
              ...
              {uploadMode === 'file' ? (
                <div className="border-2 border-dashed border-blue-200 bg-blue-50/30 rounded-2xl p-10 flex flex-col items-center text-center transition-colors hover:bg-blue-50/50 hover:border-blue-300">
                  <div className="w-14 h-14 bg-white shadow-sm rounded-full flex items-center justify-center mb-5 text-blue-600">
                    <Upload size={24} />
                  </div>
                  <h4 className="text-base font-semibold text-gray-900 mb-1">Upload a CSV or Excel file</h4>
                  ...
                  <button 
                    ...
                    disabled={loading}
                    className="px-6 py-2.5 bg-white border border-gray-200 text-gray-900 font-medium rounded-lg hover:bg-gray-50 hover:border-gray-300 transition-all text-sm shadow-sm flex items-center gap-2"
                  >
                    {loading ? <RefreshCw className="animate-spin text-blue-600" size={16} /> : <File size={16} className="text-gray-400" />}
                    Browse Files
                  </button>
                </div>
              ) : uploadMode === 'sheet' ? (
        ...
        <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in-up" style={{ animationDuration: '0.2s' }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-gray-100">
            <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              ...
              <button onClick={() => setShowCreateModal(false)} className="text-gray-400 hover:text-gray-700 transition-colors p-1 rounded-md hover:bg-gray-100">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
              </button>
            </div>
            <form onSubmit={handleCreateContact} className="p-6 space-y-5">
              ...
              <div className="pt-2 flex gap-3">
                <button 
                  ...
                  className="flex-1 px-4 py-2.5 bg-white border border-gray-200 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-all text-sm shadow-sm"
                >
                  Cancel
                </button>
                <button 
                  type="submit"
                  disabled={loading}
                  className="flex-1 px-4 py-2.5 bg-[#0070d1] text-white font-medium rounded-lg hover:bg-blue-700 transition-all text-sm shadow-sm flex items-center justify-center gap-2 disabled:opacity-50"
                >
